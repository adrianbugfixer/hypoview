...
//		Website result = this.websiteRepository.save(new Website(input.getUri()));
...
//	
	...
	ResponseEntity<?> getOne(@RequestBody Website input) {
		...
		httpHeaders.setLocation(ServletUriComponentsBuilder
				.fromCurrentRequest().path("/{id}")
				.buildAndExpand(website.getId()).toUri());
...
//	}
	...
	private void validateWebsite(String websiteUri) {
		this.websiteRepository.findByUri(websiteUri).orElseThrow(
				() -> new WebsiteNotFoundException(websiteUri));
	}
}
...
class CommentRestController {
	...
	ResponseEntity<?> add(@PathVariable Long websiteId, @RequestBody Comment input) {
		return this.websiteRepository
				.findById(websiteId)
				.map(website -> {
					Comment result = commentRepository.save(new Comment(website,
							input.content));
